...
import { useApprovedMembers, type ApprovedCoffeeChatMember } from '@/features/coffee-chats/hooks/useApprovedMembers';
...
const columns = [
  ...
  { key: 'org_position', label: 'Position' },
...
export default function CoffeeChatDirectoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return members.filter(m =>
      ...
      m.org_position?.toLowerCase().includes(q) ||
  ...
  const detailFields = [
    ...
    { label: 'Position', value: selected?.org_position },
                  ...
                  filtered.map(m => (
                    ...
                    >
                      ...
                      <td className="p-3 whitespace-nowrap">
                        {m.org_position ? <Badge variant="secondary" className="text-[10px]">{m.org_position}</Badge> : '—'}
                      </td>
