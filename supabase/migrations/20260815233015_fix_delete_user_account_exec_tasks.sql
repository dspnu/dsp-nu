-- exec_tasks.created_by is NOT NULL with ON DELETE RESTRICT, so nullifying
-- blocks account deletion. Delete tasks the user created instead.
...
BEGIN
  ...
  DELETE FROM public.exec_tasks WHERE assigned_to_user_id = _uid OR created_by = _uid;
  ...
  UPDATE public.family_bonus_points SET granted_by = NULL WHERE granted_by = _uid;
  UPDATE public.family_game_weights SET updated_by = NULL WHERE updated_by = _uid;
