CREATE OR REPLACE FUNCTION public.purge_exported_data(p_from date, p_to date, p_datasets text[])
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT (public.is_chapter_president(v_uid) OR public.is_admin_or_officer(v_uid)) THEN
