-- Fix delete_user_account: coffee_chats has initiator_id/partner_id, not user_id (was causing RPC 400).
-- Nullify optional FK references to auth.users (NO ACTION) so DELETE FROM auth.users succeeds.
