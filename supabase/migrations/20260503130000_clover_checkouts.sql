...
CREATE TABLE IF NOT EXISTS public.clover_checkouts (
...
CREATE UNIQUE INDEX IF NOT EXISTS clover_checkouts_payment_id_unique
  ON public.clover_checkouts (clover_payment_id)
  WHERE clover_payment_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS clover_checkouts_user_created_idx ON public.clover_checkouts (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS clover_checkouts_status_idx ON public.clover_checkouts (status);

DROP TRIGGER IF EXISTS clover_checkouts_updated_at ON public.clover_checkouts;
...
DROP POLICY IF EXISTS "clover_checkouts_officer_select" ON public.clover_checkouts;
...
DROP POLICY IF EXISTS "clover_checkouts_member_select_own" ON public.clover_checkouts;
